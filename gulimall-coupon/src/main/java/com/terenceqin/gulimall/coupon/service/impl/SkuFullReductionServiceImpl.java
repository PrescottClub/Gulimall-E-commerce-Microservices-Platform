...
public class SkuFullReductionServiceImpl extends ServiceImpl<SkuFullReductionDao, SkuFullReductionEntity> implements SkuFullReductionService {
    ...
    public void saveSkuReduction(SkuReductionTO skuReductionTO) {
        ...
        // 有的满减条件才保�?        if(skuReductionTO.getFullCount() > 0){
...
//        skuLadderService.save(skuLadderEntity);
        ...
        // sms_member_price  保存价格等属�?        List<MemberPrice> memberPrice = skuReductionTO.getMemberPrice();
        List<MemberPriceEntity> collect = memberPrice.stream().map(m -> {
            ...
            return priceEntity;
                ...
                // 输入的商品价格必须要大于0才保�?                (item.getMemberPrice().compareTo(new BigDecimal("0")) > 0)
